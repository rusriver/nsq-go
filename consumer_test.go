...
func TestConsumerTLS(t *testing.T) {
	for _, n := range []int{1, 10, 100, 1000} {
		...
		t.Run(topic, func(t *testing.T) {
			...
			// Create the clients used for publishing messages.
			for i, addr := range nsqd {
				nodes[i] = &Client{Address: addr, Client: httpClient, Scheme: "https"}
				// stack cleanup callbacks
				defer nodes[i].DeleteTopic(topic)
			}
...
func TestRequeueTLS(t *testing.T) {
	...
	httpClient := http.Client{Transport: &http.Transport{TLSClientConfig: tlsConf}}
	c := &Client{Address: "localhost:4152", Client: httpClient, Scheme: "https"}
	...
	consumer, _ := StartConsumer(ConsumerConfig{
		...
		Channel:  "channel",
		Lookup:   nsqlookup,
		Identify: Identify{TLSV1: true, TLSConfig: tlsConf},
	})
	defer consumer.Stop()
...
func TestDrainAndRequeueOnStopTLS(t *testing.T) {
	p, _ := NewProducer(ProducerConfig{
		Topic:        "test-stop-requeue",
		Address:      "localhost:4150",
		DialTimeout:  time.Second * 60,
